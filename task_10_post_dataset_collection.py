### Task 10
# Finish the pca_reconstruction_distance function.
# Plase collect ~500 images of the same card using the algorithm with poorly learned PCA.
# Adjust hyperparameters accordingly (PCA_MAX_DISTANCE, PCA_FIRST_K, PCA_P_NORM).
# After dataset collection run the function from task 9 with DATA_DIR='data-2'.
# After training run card_detection.py to see the card detection algorithm (do not forget to adjust hyperparameters).
                ...
                if x_points.shape[0] > 1:
                    ...
                    if rectangles.shape[0] > 0:
                        for idx in np.random.randint(rectangles.shape[0], size=2):
                            ...
                            # im = reconstructed.reshape(TO_RESIZE)
                        ...
                        if best is not None:
                            if best[2] < PCA_MAX_DISTANCE:
                                if time() - timepoint > 0.001:
                                    ...
                                    cv2.imwrite('%s/%s.jpeg' % (DATA_DIR, datetime.now().strftime("%Y-%m-%d_%H-%M-%S")), best[1])
                                    timepoint = time()
